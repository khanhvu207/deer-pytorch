...
def mean_squared_error_p(y_true, y_pred):
    ...
    return K.clip(
        K.max(K.square(y_pred - y_true), axis=-1) - 1, 0.0, 100.0
...
def exp_dec_error(y_true, y_pred):
    return K.exp(
        -5.0
        * K.sqrt(K.clip(K.sum(K.square(y_pred), axis=-1, keepdims=True), 0.000001, 10))
    )  # tend to increase y_pred
...
def loss_diff_s_s_(y_true, y_pred):
    return K.square(
        1.0 - K.sqrt(K.clip(K.sum(y_pred, axis=-1, keepdims=True), 0.000001, 1.0))
    )  # tend to increase y_pred --> loss -1
...
class CRAR(LearningAlgo):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._div_entrop_loss = kwargs.get("div_entrop_loss", 5.0)
        ...
        self.full_Q = self.learn_and_plan.full_Q_model(
            self.encoder, self.Q, 0, self._df
        )
        ...
        layers = (
            ...
            + self.R.layers
            + self.gamma.layers
            + self.transition.layers
        )

        self.params = [param for layer in layers for param in layer.trainable_weights]
        ...
        self.full_Q_target = self.learn_and_plan_target.full_Q_model(
            self.encoder_target, self.Q_target
        )
        self.full_Q_target.compile(
            optimizer="rmsprop", loss="mse"
        )  # The parameters do not matter since training is done on self.full_Q

        # Grab all the parameters of the target network together.
        layers_target = (
            self.encoder_target.layers
            + self.Q_target.layers
            + self.R_target.layers
            + self.gamma_target.layers
            + self.transition_target.layers
        )

        self.params_target = [
            param for layer in layers_target for param in layer.trainable_weights
        ]
    ...
    def train(
        ...
        if self.update_counter % 500 == 0:
            ...
            if Es.ndim == 4:
                print(
                    ...
                    np.transpose(ETs, (0, 3, 1, 2))[0],
                    np.transpose(Es_, (0, 3, 1, 2))[0],
        ...
        # Fit transition
        self.loss_T += self.diff_Tx_x_.train_on_batch(
            states_val + next_states_val + [onehot_actions] + [(1 - terminals_val)],
            np.zeros_like(Es),
        )

        # Fit rewards
        self.lossR += self.full_R.train_on_batch(
            states_val + [onehot_actions], rewards_val
        )

        # Fit gammas
        self.loss_gamma += self.full_gamma.train_on_batch(
            states_val + [onehot_actions], (1 - terminals_val[:]) * self._df
        )
        ...
        self.loss_disambiguate1 += self.encoder.train_on_batch(
            states_val, np.zeros_like(Es)
        )  # np.zeros((self._batch_size,self.learn_and_plan.internal_dim)))
        ...
        self.loss_disambiguate2 += self.encoder_diff.train_on_batch(
            [states_val[0], rolled],
            np.reshape(np.zeros_like(Es), (self._batch_size, -1)),
        )

        self.loss_disentangle_t += self.diff_s_s_.train_on_batch(
        ...
        if self._high_int_dim == False:
            ...
            target_modif_features = np.repeat(
                target_modif_features, self._batch_size, axis=0
            )
            states_val_tiled = []
            for obs in states_val:
                states_val_tiled.append(np.tile(obs, (self._n_actions, 1, 1, 1)))
            onehot_actions_tiled = np.diag(
                np.ones(self._n_actions)
            )  # np.zeros((self._batch_size*self._n_actions, self._n_actions))
            onehot_actions_tiled = np.repeat(
                onehot_actions_tiled, self._batch_size, axis=0
            )

            self.loss_interpret += self.force_features.train_on_batch(
                states_val_tiled + [onehot_actions_tiled], target_modif_features
            )
        ...
        if self._double_Q == True:
            ...
            max_next_q_vals = next_q_vals[
                np.arange(self._batch_size), argmax_next_q_vals
            ].reshape((-1, 1))
        else:
    ...
    def _compile(self):
        ...
        self.encoder.compile(optimizer=optimizer4, loss=mean_squared_error_p)
        self.encoder_diff.compile(optimizer=optimizer5, loss=exp_dec_error)

        self.diff_s_s_.compile(optimizer=optimizer6, loss=exp_dec_error)

        if self._high_int_dim == False:
            self.force_features.compile(optimizer=optimizer7, loss=cosine_proximity2)
    ...
    def setLearningRate(self, lr):
        ...
        K.set_value(
            self.diff_s_s_.optimizer.lr, self._lr / self._div_entrop_loss
        )  # /5. for simple laby or simple catcher; /1. for distrib of laby
    ...
    def transfer(self, original, transfer, epochs=1):
        ...
        for i in range(epochs):
            ...
            print("train")
            print(
                self.encoder.train_on_batch(
                    transfer[0][0 : int(size * 0.8)], x_original[0 : int(size * 0.8)]
                )
            )
            print("validation")
            print(
                self.encoder.test_on_batch(
                    transfer[0][int(size * 0.8) :], x_original[int(size * 0.8) :]
                )
            )
