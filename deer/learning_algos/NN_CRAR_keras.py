...
class NN:
    ...
    def encoder_model(self):
        ...
        for i, dim in enumerate(self._input_dimensions):
            ...
            if len(dim) == 3 or len(dim) == 4:
                if len(dim) == 4:
                    ...
                    x = Permute(
                        (2, 3, 1), input_shape=(dim[-4] * dim[-3], dim[-2], dim[-1])
                    )(
                        input
                    ...
                    inputs.append(input)
                    x = Permute((2, 3, 1), input_shape=(dim[-3], dim[-2], dim[-1]))(
                        input
                    )  # data_format='channels_last'
            ...
            # - observation[i] is a VECTOR
                ...
                if dim[-3] > 3:
                    ...
                    inputs.append(input)
                    reshaped = Reshape(
                        (dim[-3], dim[-2], 1), input_shape=(dim[-3], dim[-2])
                    )(
                        input
                    )  # data_format='channels_last'
                    x = Conv2D(16, (2, 1), activation="relu", border_mode="valid")(
                        reshaped
                    )  # Conv on the history
                    x = Conv2D(16, (2, 2), activation="relu", border_mode="valid")(
                        x
                    )  # Conv on the history & features
            ...
            # - observation[i] is a SCALAR -
                ...
                if dim[-3] > 3:
                    ...
                    inputs.append(input)
                    reshaped = Reshape((1, dim[-3], 1), input_shape=(dim[-3],))(
                        input
                    )  # data_format='channels_last'
                    x = Conv2D(8, (1, 2), activation="relu", border_mode="valid")(
                        reshaped
                    )  # Conv on the history
                    x = Conv2D(8, (1, 2), activation="relu", border_mode="valid")(
                        x
        ...
        if self._high_int_dim == False:
            ...
            x = Dense(self.internal_dim)(
                x
            )  # , activity_regularizer=regularizers.l2(0.00001))(x) #, activation='relu'
    ...
    def transition_model(self):
        ...
        if self._high_int_dim == True:
            ...
            layers_action = RepeatVector(
                (dim[-2] // self._pooling_encoder) * (dim[-1] // self._pooling_encoder)
            )(layers_action)
            layers_action = Reshape(
                (
                    (dim[-2] // self._pooling_encoder),
                    (dim[-1] // self._pooling_encoder),
                    self._n_actions,
    ...
    def diff_Tx_x_(self, encoder_model, transition_model, plan_depth=0):
        ...
        x = Multiply()(
            [x, inputs[-1]]
        )  # set to 0 if terminal because we don't care about fitting that transition
    ...
    def force_features(self, encoder_model, transition_model, plan_depth=0):
        ...
        diff_features = Subtract()(
            [Tx, enc_x]
        )  # Modification of the features after (sequence of) action(s)
    ...
    def full_float_model(
        self, encoder_model, float_model, plan_depth=0, transition_model=None
    ):
        ...
        if self._high_int_dim == True:
            ...
            for i, dim in enumerate(self._input_dimensions):
                ...
                if len(dim) == 3 or len(dim) == 4:
                    ...
                    )  # data_format is already 'channels_last'
                    inputs.append(input)
