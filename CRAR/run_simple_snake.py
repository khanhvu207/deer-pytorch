...
from simple_snake_env import MyEnv as simple_snake_env
...
if __name__ == "__main__":
    ...
    env = simple_snake_env(rng, device, higher_dim_obs=HIGHER_DIM_OBS)
    ...
    agent = NeuralAgent(
        ...
        random_state=rng,
        train_policy=None,
        test_policy=EpsilonGreedyPolicy(learning_algo, env.nActions(), rng, 1.0),
    )
    ...
    agent.attach(
        bc.FindBestController(
            validationID=simple_snake_env.VALIDATION_MODE,
            testID=None,
            unique_fname=fname,
        )
    )
    ...
    agent.attach(
        bc.InterleavedTestEpochController(
            id=simple_snake_env.VALIDATION_MODE,
