...
class NeuralAgent(object):
    ...
    def __init__(
        ...
        if test_policy is None:
            self._test_policy = EpsilonGreedyPolicy(
                learning_algo, environment.get_num_action(), random_state, 0.0
            )
        else:
    ...
    def setControllersActive(self, toDisable, active):
        """Activate controller"""
        for i in toDisable:
            self._controllers[i].set_active(active)
    ...
    def overrideNextAction(self, action):
        """Possibility to override the chosen action. This possibility should be used on the signal on_action_chosen."""
    ...
    def totalRewardOverLastTest(self):
        ...
        return (
            self._total_mode_reward / self._totalModeNbrEpisode,
            self._totalModeNbrEpisode,
        )

    def attach(self, new_controller):
        if isinstance(new_controller, controller.Controller):
            self._controllers.append(new_controller)
        else:
    ...
    def summarizeTestPerformance(self):
        ...
        self._environment.summarize_performance(
            test_data_set=self._tmp_dataset,
            learning_algo=self._learning_algo,
        ...
        try:
            if hasattr(self._learning_algo, "nstep"):
                ...
                (
                    ...
                    terminals,
                    rndValidIndices,
                ) = self._dataset.randomBatch(self._batch_size, self._exp_priority)
            ...
            if self._exp_priority:
                self._dataset.updatePriorities(
                    pow(loss_ind, self._exp_priority) + 0.0001, rndValidIndices[1]
                )
    ...
    def run(self, n_epochs, epoch_length):
        ...
        If the agent is in train mode (mode = -1):
            It starts by calling the controllers method "on_start",
    ...
    def _runEpisode(self, maxSteps):
        ...
        inputDims = self._environment.get_input_dims()
        ...
        while maxSteps > 0:
            maxSteps -= 1
            if self.gathering_data is True or self._mode != -1:
                ...
                is_terminal = self._environment.in_terminal_state()
        ...
        for c in self._controllers:
            c.on_episode_end(self, is_terminal, reward)
    ...
    def _chooseAction(self):
        if self._mode != -1:
            ...
            action, V = self._test_policy.action(
                self._state, mode=self._mode, dataset=self._dataset
            )
        else:
            if self._dataset.n_elems > self._replay_start_size:
        ...
        for c in self._controllers:
            c.on_action_chosen(self, action)
...
class DataSet(object):
    ...
    def __init__(
        ...
        self._batch_dimensions = env.get_input_dims()
        self._max_history_size = np.max(
            [self._batch_dimensions[i][0] for i in range(len(self._batch_dimensions))]
        )
        self._size = max_size
        self._use_priority = use_priority
        self._only_full_history = only_full_history

        if isinstance(env.get_num_action(), int):
        ...
        for i in range(len(self._batch_dimensions)):
            self._observations[i] = CircularBuffer(
                max_size,
                elemShape=self._batch_dimensions[i][1:],
                dtype=env.observation_type(i),
            )
    ...
    def randomBatch(self, batch_size, use_priority):
        ...
        if self._max_history_size + self.sticky_action - 1 >= self.n_elems:
            raise SliceError(
                ...
                "complete state. {} elements in dataset; requires {}".format(
                    self.n_elems, self._max_history_size
                )
            )

        if self._use_priority:
            # FIXME : take into account the case where self._only_full_history is false
            rndValidIndices, rndValidIndices_tree = self._randomPrioritizedBatch(
                batch_size
            )
            ...
            if self._only_full_history:
                for i in range(batch_size):  # TODO: multithread this loop?
                    rndValidIndices[i] = self._randomValidStateIndex(
                        self._max_history_size + self.sticky_action - 1
                    )
            else:
                for i in range(batch_size):  # TODO: multithread this loop?
                    rndValidIndices[i] = self._randomValidStateIndex(
                        minimum_without_terminal=self.sticky_action
                    )
        ...
        for input in range(len(self._batch_dimensions)):
            states[input] = np.zeros(
                (batch_size,) + self._batch_dimensions[input],
            ...
            for i in range(batch_size):
                slice = self._observations[input].getSlice(
                    ...
                    + 2
                    - min(
                        self._batch_dimensions[input][0],
                ...
                if rndValidIndices[i] >= self.n_elems - 1 or terminals[i]:
                    ...
                    slice = self._observations[input].getSlice(
                        rndValidIndices[i]
                        + 2
                        - min(self._batch_dimensions[input][0], first_terminals[i] + 1),
                        rndValidIndices[i] + 2,
                    )
                    if len(slice) == len(states[input][i]):
        ...
        if self._use_priority:
            return (
                ...
                next_states,
                terminals,
                [rndValidIndices, rndValidIndices_tree],
            )
        else:
    ...
    def randomBatch_nstep(self, batch_size, nstep, use_priority):
        ...
        for i in range(batch_size):
            actions[i] = self._actions.getSlice(
                rndValidIndices[i] - self.sticky_action * nstep + 1,
        ...
        for input in range(len(self._batch_dimensions)):
            batch_dimensions[input] = tuple(
                x + y
                for x, y in zip(
                    self._batch_dimensions[input],
                    (self.sticky_action * (nstep + 1) - 1, 0, 0),
                )
            )
            observations[input] = np.zeros(
                (batch_size,) + batch_dimensions[input],
                dtype=self._observations[input].dtype,
            )
            for i in range(batch_size):
                slice = self._observations[input].getSlice(
                    rndValidIndices[i]
                    - self.sticky_action * nstep
                    + 2
                    - min(
                        self._batch_dimensions[input][0],
                        first_terminals[i] - self.sticky_action * nstep + 1,
                    ),
    ...
    def _randomPrioritizedBatch(self, batch_size):
        indices_tree = self._prioritiy_tree.getBatch(
            batch_size, self._random_state, self
        )
        indices_replay_mem = np.zeros(indices_tree.size, dtype="int32")
        for i in range(len(indices_tree)):
            indices_replay_mem[i] = int(
                self._translation_array[indices_tree[i]] - self._actions.getLowerBound()
            )
