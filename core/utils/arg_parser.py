...
def process_args(args, defaults):
    ...
    parser.add_argument(
        "-f",
        "--freq_summary_perfs",
        dest="period_btw_summary_perfs",
        type=int,
        default=defaults.PERIOD_BTW_SUMMARY_PERFS,
    ...
    parser.add_argument(
        ...
        help=("Max L2 norm for the gradient. " + "(default: %(default)s)"),
    )
    parser.add_argument(
        "--discount", type=float, default=defaults.DISCOUNT, help="Discount rate init"
    )
    parser.add_argument(
        "--discount_inc",
        type=float,
        default=defaults.DISCOUNT_INC,
        help="Discount rate",
    )
    parser.add_argument(
        "--discount_max",
        type=float,
        default=defaults.DISCOUNT_MAX,
    ...
    parser.add_argument(
        ...
        help=(
            "Maximum number of steps stored in replay "
    ...
    parser.add_argument(
        ...
        dest="online_logger",
        type=int,
        default=defaults.WANDB_ONLINE_MODE,
        help="Log training to the cloud via W&B (default: %(default)s)",
    )

    parser.set_defaults(deterministic=defaults.DETERMINISTIC)
    parser.add_argument(
        "--param1", dest="param1"
    )  # Additional parameter depending on the environment
    parser.add_argument(
        "--param2", dest="param2"
    )  # Additional parameter depending on the environment
    parser.add_argument(
